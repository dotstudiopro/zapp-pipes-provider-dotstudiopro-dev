...
var authenticate = function (nativeBridge, api_key) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    const token = JSON.parse(nativeBridge.getLocalStoreItem("token", "dotstudiopro"));
    const expires = JSON.parse(nativeBridge.getLocalStoreItem("expiry", "dotstudiopro"));
...
function getAccessToken(api_key, cb) {
  const returnObj = {};
  axios.post(`https://api.myspotlight.tv/token?key=${api_key}`).then(response => {
